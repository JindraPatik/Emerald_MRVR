...
void UIsdkGrabbableComponent::BeginPlay()
{
  ...
  GrabTransformer = GetOwner()->FindComponentByClass<UIsdkGrabTransformerComponent>();
...
void UIsdkGrabbableComponent::PostEvent(const FIsdkInteractionPointerEvent& Event)
{
  ...
  if (IsValid(GrabTransformer.GetObject()))
  {
    GrabTransformer->ProcessPointerEvent(Event);
  }
}
...
bool UIsdkGrabbableComponent::IsGrabTypeAllowed(EIsdkGrabColliderType Type)
{
  return GrabTypesAllowed.Contains(Type);
}

void UIsdkGrabbableComponent::SetGrabTypeAllowed(EIsdkGrabColliderType Type, bool Allowed)
{
...
TScriptInterface<IIsdkIGrabbable> UIsdkGrabbableComponent::GetGrabTransformer() const
{
